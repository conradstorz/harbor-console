...
- **Traefik, Docker, or the host's own socket table being unreadable is a banner, never a refusal.** The page still renders from what it does have, says in the banner which evidence is missing, and withholds exactly the findings that evidence supported. A cycle that fails outright leaves the last good snapshot standing with the reason attached, and a successful cycle clears it.
