...
services:
  traefik:
    ...
    volumes:
      ...
      - /etc/traefik/dashboard-users:/etc/traefik/dashboard-users:ro
    networks:
      - harbor
      - admin
    labels:
...
**`--entrypoints.traefik.address=:8081`** — a third entrypoint, dedicated to
Traefik's own read-only API. The `ports:` section below publishes it on
`127.0.0.1:8081` only, so nothing on the tailnet or the LAN can reach it
directly — but that publish only constrains how the *host* reaches it. The
same entrypoint answers identically as `traefik:8081` to any other container
on `harbor`, since Docker's bridge networking makes no distinction between
"arrived via the host's published port" and "arrived from a sibling
container." An earlier version of this file (`--api.insecure=true`) served
the API there with no authentication at all — read-only or not, that handed
over the whole edge configuration, every routed hostname and backend
included, to anything running on `harbor`. ADR 16 closes this: `--api=true`
...
### The gated API (ADR 16)

`dynamic/api.yml` is a second, static file-provider file (no
`@TAILNET_ADDRESS@`-style templating needed, since nothing in it is
host-specific) that declares a router for Traefik's own API:
...
`service: api@internal` points at Traefik's own built-in API service, which
`--api=true` (not `--api.insecure=true`) makes *available* without
auto-creating a router for it — this router is that missing piece, declared
explicitly so it can carry an auth middleware. The `basicAuth` middleware
checks credentials against an htpasswd-format file, `/etc/traefik/dashboard-users`,
generated by `install.sh` and mounted read-only into the container.

The one legitimate caller is `harbor-console-web` itself
(`harbor_console.traefik.traefik_routers`), which reads the *plaintext*
password from a second file, `/etc/traefik/dashboard-password` — deliberately
not the same file Traefik reads, and not `/etc/traefik/env` either. Three
separate files, three separate reasons:

- `/etc/traefik/env` holds the Cloudflare API token — root-only, mode 0600,
  read only by `install.sh` (as root) and mounted into the Traefik container.
- `/etc/traefik/dashboard-users` holds the htpasswd *hash* — root-only,
  read only by the Traefik container.
- `/etc/traefik/dashboard-password` holds the same password in *plaintext*
  — owned `root:harbor`, mode 0640, because `harbor-console-web` runs as the
  unprivileged `harbor` user, not root, and needs to present the credential
  itself rather than verify someone else's.

`install.sh` generates the password once (printed to the terminal exactly
that one time — it is not recoverable afterward except by reading
`/etc/traefik/env` as root) and derives both other files from it on every
run, so a hand-edited password in `/etc/traefik/env` takes effect on the
next `install.sh` without any other step.

A wrong or missing credential fails exactly like an unreachable API always
has: `traefik_routers` gets a 401, `urlopen` raises, `TRAEFIK_UNAVAILABLE`
comes back, the page shows its banner. Nothing about the failure mode
changed — only what it takes to succeed.
...
1. **Prerequisite checks, before anything is touched**: `docker`, `tailscale`
   and `openssl` are on PATH; if the `harbor` Docker network already exists,
   it must carry the fixed bridge name `br-harbor` (see "The ufw problem"
   below) or the script refuses and explains how to recreate it;
   `/etc/traefik/env` exists; `ACME_EMAIL` is set within it (parsed with
   `grep`, never sourced — see "Secrets" below for why); `tailscale ip -4`
...
4. **Prepare the edge**: `chmod 0600 /etc/traefik/env` (belt-and-braces, the
   file should already be 0600); create the `harbor` and `admin` Docker
   networks if they don't exist yet; generate or re-derive the dashboard
   credential (previous section); write `deploy/traefik/.env` from
   `TAILNET_ADDRESS` and `ACME_EMAIL`; render `harbor.yml` from its
   template; `docker compose up -d --remove-orphans` in `deploy/traefik/`;
   *then*, once Traefik's address on `harbor` is known, apply the `ufw` rule
   (next section).
...
...
traefik_harbor_ip=$(docker inspect traefik -f '{{(index .NetworkSettings.Networks "harbor").IPAddress}}')
ufw allow in on br-harbor from "${traefik_harbor_ip}" to "${TAILNET_ADDRESS}" port 8100 proto tcp comment 'harbor-console: traefik -> page'
```
...
2. **`on br-harbor` alone is not enough** (ADR 16). An interface match
   admits traffic from *any* source reaching the host via that interface —
   every container on `harbor`, not only Traefik. The rule needs `from
   <traefik's own address>` too. `harbor`'s subnet was fixed the moment
   Docker first created it (it auto-assigned `172.25.0.0/16` here, and
   nothing since has deleted and recreated the network to change that), so
   Traefik's own address on it is stable without needing to pin one in
   compose — `install.sh` just asks Docker for it, *after* starting Traefik,
   each time it runs. The rule is applied idempotently: any earlier instance
   (matched by its comment) is deleted first, so a stale allow for an
   address nothing uses any more never lingers alongside a fresh one.
3. The rule is otherwise scoped as tightly as the requirement allows: **to**
   the tailnet address specifically (not `any`), **port 8100** specifically
   (not the whole host). This is the narrowest rule that solves the actual
   problem — every *other* container-to-host path on this host remains
   exactly as `ufw` denies it by default.
...
- **`admin`, not `harbor` (ADR 16).** Portainer holds a read-write Docker
  socket, which is equivalent to root on the host — anything with API access
  to it can start a container that bind-mounts `/` and walks straight out.
  `harbor` is the network every routed *project* container now joins; giving
  every one of them a network path to Portainer's own API would have made
  every one of them one compromise away from full host takeover. `admin` is
  a second network, created by `install.sh` alongside `harbor`, that only
  Traefik and Portainer ever join — no project container is ever on it.
  Traefik joins both because it is the trusted routing component, not an
  untrusted workload, and `traefik.docker.network: admin` on Portainer's own
  labels tells Traefik which of its two networks to actually route that
  backend over (Traefik's Docker provider otherwise defaults to `harbor`, per
  `--providers.docker.network=harbor`).
- **`traefik.http.routers.portainer.entrypoints: websecure`, added at the
  same time.** Without it, Traefik's default for a router with no explicit
  `entryPoints` is *every configured entrypoint* — so the router was also
  being served on the plaintext, unauthenticated-by-TLS `traefik` entrypoint
  (`:8081`), reachable from `admin` with no certificate involved at all. The
  file-provider route for the status page always pinned `entryPoints:
  [websecure]` explicitly (see `harbor.yml.in` above); this was the one
  container label set that hadn't matched that pattern.
...
| `deploy/traefik/dynamic/api.yml` | The gated `/api` router and its auth middleware (ADR 16) | Yes — host-specific only in the file it references, not in itself. |
| `/etc/traefik/dashboard-users` | Traefik's API password, as an htpasswd hash | **No.** Root-only, mode 0600. Derived from `TRAEFIK_DASHBOARD_PASSWORD` in `/etc/traefik/env` by `install.sh` on every run. |
| `/etc/traefik/dashboard-password` | The same password, in plaintext | **No.** Owned `root:harbor`, mode 0640 — `harbor-console-web` runs as `harbor`, not root, and needs to read this one itself. |
...
```
Expect the first to show the `br-harbor` rule, `from` Traefik's current
address on `harbor` (`docker inspect traefik -f '{{(index .NetworkSettings.Networks "harbor").IPAddress}}'`
should match), and the second to print `br-harbor`. If the second prints
anything else (or an empty string), the network needs to be recreated per
the error message `install.sh` prints in this situation. The rule's `from`
address is self-healing on every `install.sh` run (ADR 16) — if Traefik's
address ever changed and the rule looks stale, re-running the installer is
the fix, not hand-editing `ufw`.
...
- *Traefik reports the router as `disabled`, with its own error text* — a
  ...
  malformed `Host()` rule. Read Traefik's own error (the API is gated by
  Basic Auth since ADR 16, so the credential from `/etc/traefik/dashboard-password`
  goes on the request):
  ```bash
  ssh gte@hpz440 'sudo sh -c "curl -s -u harbor:\$(cat /etc/traefik/dashboard-password) http://127.0.0.1:8081/api/http/routers | python3 -m json.tool"'
  ```
